...
module "project_setup" {
  ...
  region                            = var.region
  artifact_registry_repository_name = "fms-app-repo" # You can change this name
}
...
module "cloud_run_service" {
  ...
  project_id             = var.project_id
  region                 = var.region
  cloud_run_service_name = "fms-data-pipeline-rnd"
  image_url              = "${var.region}-docker.pkg.dev/${var.project_id}/${module.project_setup.repository_url}/fms-data-pipeline:latest"
  service_account_id     = "fms-data-pipeline-rnd-sa"
  gcs_bucket_name        = "fms-data-pipeline-rnd-events"
  eventarc_trigger_name  = "fms-data-pipeline-rnd-trigger"
}
